...
class WeatherState extends Equatable {
  ...
  List<DayWeatherParams> get days{
    List<DayWeatherParams> _days = List.generate(
        (presentFutureWeather?.daily?.length ?? 0), (index) => DayWeatherParams(
            ...
            windDeg: presentFutureWeather?.daily
                .tryElementAt(index)
                ?.windDeg?.toString() ??"",
            pressure: presentFutureWeather?.daily
                .tryElementAt(index)
                ?.pressure?.toString() ??"",
            clouds: presentFutureWeather?.daily
                .tryElementAt(index)
                ?.clouds?.toString() ??"",
            uvi: presentFutureWeather?.daily
                .tryElementAt(index)
                ?.uvi?.toString() ??"",
            humidity:presentFutureWeather?.daily
                .tryElementAt(index)
                ?.humidity?.toString() ??"",
            visibility: "visibility",
            detailedDescription: presentFutureWeather!.daily
                .tryElementAt(index)?.weather.tryFirst?.description.toString() ?? "",
            feelsLike: "feelsLike",
            isImageNetwork: true,
